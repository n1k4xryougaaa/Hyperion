...
# https://github.com/n1k4xryougaaa/Hyperion
...
try:
    if (
        ...
        __authors__ != ("n1k4xryougaaa",) or
        __github__ != "https://github.com/n1k4xryougaaa/Hyperion" or
...
""".strip()
    ...
    def _randvar2(self):
        return ''.join(choice('n1k4xryougaaa') for _ in range(randint(5, 20)))
...
def stage(text: str, symbol: str = '...', col1 = Col.light_gray, col2 = None) -> str:
    if col2 is None:
        col2 = Col.light_gray if symbol == '...' else Colors.StaticMIX((Col.purple, Col.blue))
    return f""" {Col.Symbol(symbol, col1, Col.dark_gray)} {col2}{text}{Col.reset}"""

def main():
    System.Size(150, 47)
    System.Title("Hyperion")

    file = input(stage(f"Drag the file you want to obfuscate {Col.dark_gray}-> {Col.reset}", "?", col2 = Colors.StaticMIX((Col.purple, Col.blue, Col.blue)))).replace('"','').replace("'","")
    print('\n')
    ...
    print('\n')
    getpass(stage(f"Obfuscation completed succesfully in {Col.light_gray}{now}s{Colors.StaticMIX((Col.purple, Col.blue, Col.blue))}.{Col.reset}", "?", col2 = Colors.StaticMIX((Col.purple, Col.blue, Col.blue))))
